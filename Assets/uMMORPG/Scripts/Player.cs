...
public partial class Player : Entity
{
    ...
    string UpdateServer_MOVING()
    {
        ...
        if (EventCancelAction())
        {
            ...
            currentSkill = -1;
            //agent.ResetMovement(); <- done locally. doing it here would reset localplayer to the slightly behind server position otherwise
            return "IDLE";
        }
        if (EventTradeStarted())
        {
        ...
        if (EventSkillRequest())
        {
            ...
            if (!IsMounted())
            {
                ...
                if (CastCheckSelf(skill) && CastCheckTarget(skill) && CastCheckDistance(skill, out destination))
                {
                    ...
                    // agent.ResetMovement(); <- DO NOT DO THIS.
                    StartCastSkill(skill);
                    return "CASTING";
                }
            }
